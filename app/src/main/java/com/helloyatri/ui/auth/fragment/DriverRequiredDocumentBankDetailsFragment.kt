...
import com.helloyatri.databinding.AuthDriverCompleteBankDetailsFragmentBinding
...
@AndroidEntryPoint
class DriverRequiredDocumentBankDetailsFragment :
        BaseFragment<AuthDriverCompleteBankDetailsFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverRequiredDocumentsBankDetailsAdapter by lazy {
    ...
    private val driverRequiredDocumentsBankDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverCompleteBankDetailsFragmentBinding {
        return AuthDriverCompleteBankDetailsFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverRequiredDocumentsBankDetailsAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        buttonSave.setOnClickListener {
            session.isBankAccountDetailsAdded = true
            navigator.goBack()
        }
    }
    ...
    private fun setUpData() {
        driverRequiredDocumentsBankDetailsList.clear()
        driverRequiredDocumentsBankDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverRequiredDocumentsBankDetailsList.add(DriverProfilePictureDetails(
        ...
        driverRequiredDocumentsBankDetailsAdapter.setItems(driverRequiredDocumentsBankDetailsList,
                1)
