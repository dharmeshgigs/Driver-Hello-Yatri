...
import com.helloyatri.databinding.AuthDriverCompleteDriverLicenceFragmentBinding
...
@AndroidEntryPoint
class DriverRequiredDocumentLicenceFragment :
        BaseFragment<AuthDriverCompleteDriverLicenceFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverRequiredDocumentsLicenseDetailsAdapter by lazy {
        DriverProfilePictureDetailsAdapter()
    }

    private val driverRequiredDocumentsLicenseImagesAdapter by lazy {
        DriverProfilePictureImagesAdapter()
    }

    private val driverRequiredDocumentsLicenseDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverCompleteDriverLicenceFragmentBinding {
        return AuthDriverCompleteDriverLicenceFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverRequiredDocumentsLicenseDetailsAdapter
            layoutManager = LinearLayoutManager(requireContext(), RecyclerView.VERTICAL, false)
        }

        recyclerViewImages.apply {
            adapter = driverRequiredDocumentsLicenseImagesAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        driverRequiredDocumentsLicenseImagesAdapter.setOnItemClickListener {
            driverRequiredDocumentsLicenseImagesAdapter.removeItem(it)
            updateCount()
        }

        buttonSave.setOnClickListener {
            session.isDrivingLicenseAdded = true
            navigator.goBack()
        }
    }
        ...
        mediaSelectHelper.registerCallback(object : MediaSelector {
            override fun onAnyFileSelected(outPutFileAny: OutPutFileAny) {
                ...
                when (outPutFileAny.type) {
                    FileType.Image -> {
                        driverRequiredDocumentsLicenseImagesAdapter.isBitMap = false
                        driverRequiredDocumentsLicenseImagesAdapter.addItem(
                    ...
                    FileType.Pdf -> {
                        requireActivity().contentResolver.openFileDescriptor(outPutFileAny.uri, "r")
                                ?.use { p ->
                                    ...
                                    pdfRenderer.close()
                                    driverRequiredDocumentsLicenseImagesAdapter.isBitMap = true
                                    driverRequiredDocumentsLicenseImagesAdapter.addItem(
    ...
    private fun setUpData() {
        driverRequiredDocumentsLicenseDetailsList.clear()
        driverRequiredDocumentsLicenseDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverRequiredDocumentsLicenseDetailsList.add(DriverProfilePictureDetails(
        ...
        driverRequiredDocumentsLicenseDetailsAdapter.setItems(
                driverRequiredDocumentsLicenseDetailsList, 1)

        driverRequiredDocumentsLicenseImagesAdapter.addItem(DriverProfilePictureImages(
                images = "https://images.unsplash.com/photo-1623795457659-f6b530b2e383?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"))
        updateCount()
    }

    private fun updateCount() = with(binding) {
        if (driverRequiredDocumentsLicenseImagesAdapter.items?.isEmpty()?.not() == true) {
            recyclerViewImages.show()
            textViewUploadDocuments.text = getString(R.string.label_s_documents_uploaded,
                    driverRequiredDocumentsLicenseImagesAdapter.itemCount.toString())
