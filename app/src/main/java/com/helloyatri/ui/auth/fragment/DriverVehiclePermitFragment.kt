...
@AndroidEntryPoint
class DriverVehiclePermitFragment : BaseFragment<AuthDriverVehiclePermitFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverVehiclePermitDetailsAdapter by lazy {
        DriverProfilePictureDetailsAdapter()
    }

    private val driverVehiclePermitImagesAdapter by lazy {
        DriverProfilePictureImagesAdapter()
    }

    private val driverVehiclePermitDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverVehiclePermitFragmentBinding {
        return AuthDriverVehiclePermitFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverVehiclePermitDetailsAdapter
            layoutManager = LinearLayoutManager(requireContext(), RecyclerView.VERTICAL, false)
        }

        recyclerViewImages.apply {
            adapter = driverVehiclePermitImagesAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        driverVehiclePermitImagesAdapter.setOnItemClickListener {
            driverVehiclePermitImagesAdapter.removeItem(it)
            updateCount()
        }

        buttonSave.setOnClickListener {
            session.isVehiclePermitAdded = true
            navigator.goBack()
        }
    }
                    ...
                    FileType.Pdf -> {
                        requireActivity().contentResolver.openFileDescriptor(outPutFileAny.uri, "r")
                                ?.use { p ->
                                    ...
                                    pdfRenderer.close()
                                    driverVehiclePermitImagesAdapter.isBitMap = true
                                    driverVehiclePermitImagesAdapter.addItem(
    ...
    private fun setUpData() {
        driverVehiclePermitDetailsList.clear()
        driverVehiclePermitDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverVehiclePermitDetailsList.add(DriverProfilePictureDetails(
    ...
    private fun updateCount() = with(binding) {
        if (driverVehiclePermitImagesAdapter.items?.isEmpty()?.not() == true) {
            recyclerViewImages.show()
            textViewUploadDocuments.text = getString(R.string.label_s_documents_uploaded,
                    driverVehiclePermitImagesAdapter.itemCount.toString())
