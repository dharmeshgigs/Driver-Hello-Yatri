...
import com.helloyatri.databinding.AuthDriverVehiclePhotoChassisFragmentBinding
...
@AndroidEntryPoint
class DriverVehiclePhotoChassisFragment :
        BaseFragment<AuthDriverVehiclePhotoChassisFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverVehiclePhotoChassisDetailsAdapter by lazy {
        DriverProfilePictureDetailsAdapter()
    }

    private val driverVehiclePhotoChassisImagesAdapter by lazy {
        DriverProfilePictureImagesAdapter()
    }

    private val driverVehiclePhotoChassisDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverVehiclePhotoChassisFragmentBinding {
        return AuthDriverVehiclePhotoChassisFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverVehiclePhotoChassisDetailsAdapter
            layoutManager = LinearLayoutManager(requireContext(), RecyclerView.VERTICAL, false)
        }

        recyclerViewImages.apply {
            adapter = driverVehiclePhotoChassisImagesAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        driverVehiclePhotoChassisImagesAdapter.setOnItemClickListener {
            driverVehiclePhotoChassisImagesAdapter.removeItem(it)
            updateCount()
        }

        buttonSave.setOnClickListener {
            session.isVehicleChassisAdded = true
            navigator.goBack()
        }
    }
        ...
        mediaSelectHelper.registerCallback(object : MediaSelector {
            override fun onAnyFileSelected(outPutFileAny: OutPutFileAny) {
                ...
                when (outPutFileAny.type) {
                    FileType.Image -> {
                        driverVehiclePhotoChassisImagesAdapter.isBitMap = false
                        driverVehiclePhotoChassisImagesAdapter.addItem(
                    ...
                    FileType.Pdf -> {
                        requireActivity().contentResolver.openFileDescriptor(outPutFileAny.uri, "r")
                                ?.use { p ->
                                    ...
                                    pdfRenderer.close()
                                    driverVehiclePhotoChassisImagesAdapter.isBitMap = true
                                    driverVehiclePhotoChassisImagesAdapter.addItem(
    ...
    private fun setUpData() {
        driverVehiclePhotoChassisDetailsList.clear()
        driverVehiclePhotoChassisDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverVehiclePhotoChassisDetailsList.add(DriverProfilePictureDetails(
        ...
        driverVehiclePhotoChassisDetailsAdapter.setItems(driverVehiclePhotoChassisDetailsList, 1)

        driverVehiclePhotoChassisImagesAdapter.addItem(DriverProfilePictureImages(
                images = "https://images.unsplash.com/photo-1696494561079-ddabcbb308e8?q=80&w=2072&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"))
        updateCount()
    }

    private fun updateCount() = with(binding) {
        if (driverVehiclePhotoChassisImagesAdapter.items?.isEmpty()?.not() == true) {
            recyclerViewImages.show()
            textViewUploadDocuments.text = getString(R.string.label_s_documents_uploaded,
                    driverVehiclePhotoChassisImagesAdapter.itemCount.toString())
