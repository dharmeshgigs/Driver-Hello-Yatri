...
import com.helloyatri.databinding.AuthDriverVehiclePhotoFrontBackFragmentBinding
...
@AndroidEntryPoint
class DriverVehiclePhotoFrontBackFragment :
        BaseFragment<AuthDriverVehiclePhotoFrontBackFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverVehiclePhotoFrontBackDetailsAdapter by lazy {
        DriverProfilePictureDetailsAdapter()
    }

    private val driverVehiclePhotoFrontBackImagesAdapter by lazy {
        DriverProfilePictureImagesAdapter()
    }

    private val driverVehiclePhotoFrontBackDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverVehiclePhotoFrontBackFragmentBinding {
        return AuthDriverVehiclePhotoFrontBackFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverVehiclePhotoFrontBackDetailsAdapter
            layoutManager = LinearLayoutManager(requireContext(), RecyclerView.VERTICAL, false)
        }

        recyclerViewImages.apply {
            adapter = driverVehiclePhotoFrontBackImagesAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        driverVehiclePhotoFrontBackImagesAdapter.setOnItemClickListener {
            driverVehiclePhotoFrontBackImagesAdapter.removeItem(it)
            updateCount()
        }

        buttonSave.setOnClickListener {
            session.isVehicleFrontBackPhotoAdded = true
            navigator.goBack()
        }
    }
        ...
        mediaSelectHelper.registerCallback(object : MediaSelector {
            override fun onAnyFileSelected(outPutFileAny: OutPutFileAny) {
                ...
                when (outPutFileAny.type) {
                    FileType.Image -> {
                        driverVehiclePhotoFrontBackImagesAdapter.isBitMap = false
                        driverVehiclePhotoFrontBackImagesAdapter.addItem(
                    ...
                    FileType.Pdf -> {
                        requireActivity().contentResolver.openFileDescriptor(outPutFileAny.uri, "r")
                                ?.use { p ->
                                    ...
                                    pdfRenderer.close()
                                    driverVehiclePhotoFrontBackImagesAdapter.isBitMap = true
                                    driverVehiclePhotoFrontBackImagesAdapter.addItem(
    ...
    private fun setUpData() {
        driverVehiclePhotoFrontBackDetailsList.clear()
        driverVehiclePhotoFrontBackDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverVehiclePhotoFrontBackDetailsList.add(DriverProfilePictureDetails(
        ...
        driverVehiclePhotoFrontBackDetailsAdapter.setItems(driverVehiclePhotoFrontBackDetailsList,
                1)

        driverVehiclePhotoFrontBackImagesAdapter.addItem(DriverProfilePictureImages(
                images = "https://images.unsplash.com/photo-1618850918792-aeccf6c98152?q=80&w=1780&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"))
        updateCount()
    }

    private fun updateCount() = with(binding) {
        if (driverVehiclePhotoFrontBackImagesAdapter.items?.isEmpty()?.not() == true) {
            recyclerViewImages.show()
            textViewUploadDocuments.text = getString(R.string.label_s_documents_uploaded,
                    driverVehiclePhotoFrontBackImagesAdapter.itemCount.toString())
