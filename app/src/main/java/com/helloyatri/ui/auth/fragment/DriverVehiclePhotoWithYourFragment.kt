...
import com.helloyatri.databinding.AuthDriverVehiclePhotoWithYourFragmentBinding
...
@AndroidEntryPoint
class DriverVehiclePhotoWithYourFragment :
        BaseFragment<AuthDriverVehiclePhotoWithYourFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverVehiclePhotoWithYourDetailsAdapter by lazy {
        DriverProfilePictureDetailsAdapter()
    }

    private val driverVehiclePhotoWithYourImagesAdapter by lazy {
        DriverProfilePictureImagesAdapter()
    }

    private val driverVehiclePhotoWithYourDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverVehiclePhotoWithYourFragmentBinding {
        return AuthDriverVehiclePhotoWithYourFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverVehiclePhotoWithYourDetailsAdapter
            layoutManager = LinearLayoutManager(requireContext(), RecyclerView.VERTICAL, false)
        }

        recyclerViewImages.apply {
            adapter = driverVehiclePhotoWithYourImagesAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        driverVehiclePhotoWithYourImagesAdapter.setOnItemClickListener {
            driverVehiclePhotoWithYourImagesAdapter.removeItem(it)
            updateCount()
        }

        buttonSave.setOnClickListener {
            session.isVehicleWithYourPhotoAdded = true
            navigator.goBack()
        }
    }
        ...
        mediaSelectHelper.registerCallback(object : MediaSelector {
            override fun onAnyFileSelected(outPutFileAny: OutPutFileAny) {
                ...
                when (outPutFileAny.type) {
                    FileType.Image -> {
                        driverVehiclePhotoWithYourImagesAdapter.isBitMap = false
                        driverVehiclePhotoWithYourImagesAdapter.addItem(
                    ...
                    FileType.Pdf -> {
                        requireActivity().contentResolver.openFileDescriptor(outPutFileAny.uri, "r")
                                ?.use { p ->
                                    ...
                                    pdfRenderer.close()
                                    driverVehiclePhotoWithYourImagesAdapter.isBitMap = true
                                    driverVehiclePhotoWithYourImagesAdapter.addItem(
    ...
    private fun setUpData() {
        driverVehiclePhotoWithYourDetailsList.clear()
        driverVehiclePhotoWithYourDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverVehiclePhotoWithYourDetailsList.add(DriverProfilePictureDetails(
        ...
        driverVehiclePhotoWithYourDetailsAdapter.setItems(driverVehiclePhotoWithYourDetailsList, 1)

        driverVehiclePhotoWithYourImagesAdapter.addItem(DriverProfilePictureImages(
                images = "https://images.unsplash.com/photo-1520975661595-6453be3f7070?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"))
        updateCount()
    }

    private fun updateCount() = with(binding) {
        if (driverVehiclePhotoWithYourImagesAdapter.items?.isEmpty()?.not() == true) {
            recyclerViewImages.show()
            textViewUploadDocuments.text = getString(R.string.label_s_documents_uploaded,
                    driverVehiclePhotoWithYourImagesAdapter.itemCount.toString())
