...
import com.helloyatri.databinding.AuthDriverVehicleRegistrationFragmentBinding
...
@AndroidEntryPoint
class DriverVehicleRegistrationCertificateFragment :
        BaseFragment<AuthDriverVehicleRegistrationFragmentBinding>() {

    @Inject
    lateinit var mediaSelectHelper: MediaSelectHelper

    private val driverVehicleRegistrationCertificateDetailsAdapter by lazy {
        DriverProfilePictureDetailsAdapter()
    }

    private val driverVehicleRegistrationImagesAdapter by lazy {
        DriverProfilePictureImagesAdapter()
    }

    private val driverVehicleRegistrationDetailsList = ArrayList<DriverProfilePictureDetails>()

    override fun createViewBinding(inflater: LayoutInflater, container: ViewGroup?,
                                   attachToRoot: Boolean): AuthDriverVehicleRegistrationFragmentBinding {
        return AuthDriverVehicleRegistrationFragmentBinding.inflate(layoutInflater)
    }
    ...
    private fun setUpRecyclerView() = with(binding) {
        recyclerViewDetails.apply {
            adapter = driverVehicleRegistrationCertificateDetailsAdapter
            layoutManager = LinearLayoutManager(requireContext(), RecyclerView.VERTICAL, false)
        }

        recyclerViewImages.apply {
            adapter = driverVehicleRegistrationImagesAdapter
    ...
    private fun setUpClickListener() = with(binding) {
        ...
        driverVehicleRegistrationImagesAdapter.setOnItemClickListener {
            driverVehicleRegistrationImagesAdapter.removeItem(it)
            updateCount()
        }

        buttonSave.setOnClickListener {
            session.isVehicleRegistrationAdded = true
            navigator.goBack()
        }
    }
        ...
        mediaSelectHelper.registerCallback(object : MediaSelector {
            override fun onAnyFileSelected(outPutFileAny: OutPutFileAny) {
                ...
                when (outPutFileAny.type) {
                    FileType.Image -> {
                        driverVehicleRegistrationImagesAdapter.isBitMap = false
                        driverVehicleRegistrationImagesAdapter.addItem(
                    ...
                    FileType.Pdf -> {
                        requireActivity().contentResolver.openFileDescriptor(outPutFileAny.uri, "r")
                                ?.use { p ->
                                    ...
                                    pdfRenderer.close()
                                    driverVehicleRegistrationImagesAdapter.isBitMap = true
                                    driverVehicleRegistrationImagesAdapter.addItem(
    ...
    private fun setUpData() {
        driverVehicleRegistrationDetailsList.clear()
        driverVehicleRegistrationDetailsList.add(DriverProfilePictureDetails(text = getString(
                R.string.label_photocopies_and_printouts_of_documents_will_not_be_accepted)))
        driverVehicleRegistrationDetailsList.add(DriverProfilePictureDetails(
        ...
        driverVehicleRegistrationCertificateDetailsAdapter.setItems(
                driverVehicleRegistrationDetailsList, 1)

        driverVehicleRegistrationImagesAdapter.addItem(DriverProfilePictureImages(
    ...
    private fun updateCount() = with(binding) {
        if (driverVehicleRegistrationImagesAdapter.items?.isEmpty()?.not() == true) {
            recyclerViewImages.show()
            textViewUploadDocuments.text = getString(R.string.label_s_documents_uploaded,
                    driverVehicleRegistrationImagesAdapter.itemCount.toString())
