...
@AndroidEntryPoint
class CancelledRideFragment : BaseFragment<AllRideStatusFragmentBinding>() {
    ...
    private fun initObservers() {
        apiViewModel.getCancelledRideLiveData.observe(viewLifecycleOwner) { resource ->
            resource?.let {
                when (resource.status) {
                    com.helloyatri.network.Status.SUCCESS -> {
                        hideProgressBar()
                        allRidesStatusAdapter.setItems(apiViewModel.cancelledTrips.toList(), 1)
                        showPlaceHolder()
                    }
    ...
    override fun onResume() {
        ...
        rideStatus?.let {
            apiViewModel.getCancelledRideData(mutableMapOf<String, String>().apply {
                put(Constants.PARAM_FILTER_PARAMETER, "CANCELLED")
            })
        }
    }
