...
class RatingBar : FrameLayout, View.OnClickListener {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs == null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                mFillDrawable = context.getDrawable(R.drawable.ic_star_filled)
                mEmptyDrawable = context.getDrawable(R.drawable.ic_star_unfilled)
            } else {
                mFillDrawable = context.getDrawable(R.drawable.ic_star_filled)
                mEmptyDrawable = context.getDrawable(R.drawable.ic_star_unfilled)
            }
            ...
            if (mFillDrawable == null) {
                mFillDrawable = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    context.getDrawable(R.drawable.ic_star_filled)
                } else {
                    context.resources.getDrawable(R.drawable.ic_star_filled)
                }
            }
        ...
        addView(
            mRootLayout, ViewGroup.LayoutParams(
    ...
    private fun updateStarViews() {
        ...
        for (i in mImageViews!!.indices) {
            ...
            val mlp = parent.layoutParams as MarginLayoutParams
            if (imgView.drawable != null && i - 1 >= 0 && mImageViews!![i - 1]?.drawable != null) {
