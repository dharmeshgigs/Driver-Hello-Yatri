package com.helloyatri.utils.circularprogressbar
...
class CircularSeekBar @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null,
    ...
    private val typedArray by lazy {
        getContext().obtainStyledAttributes(attrs, R.styleable.CircularSeekBar)
    }

    private var strokeWidth =
            typedArray.getFloat(R.styleable.CircularSeekBar_progressStrokeWidth, STROKE_WIDTH)

    private var selectionScale =
            typedArray.getFloat(R.styleable.CircularSeekBar_selectionScale, SELECTION_SCALE)
    ...
    var progress: Long = 0
        set(value) {
            ...
            if (isNew) seekListener?.onProgressChanged(this, field, false)
        }
    ...
    init {
        ...
        mProgressPaint = Paint().apply {
            strokeWidth = sDP * this@CircularSeekBar.strokeWidth
     ...
     */
    fun setPrimaryColor(primaryColor: Int, animate: Boolean = true) {
        // val toColor = ContextCompat.getColor(context, primaryColor)
        if (animate) {
            mProgressPaint.animateColorChange(primaryColor)
            mThumbPaint.animateColorChange(primaryColor)
        } else {
            this.mPrimaryColor = primaryColor
            mProgressPaint.color = primaryColor
            mThumbPaint.color = primaryColor
            invalidate()
        }
    }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        mOvalRect = if (w >= h) {
            val top = selectionScale * mPressedThumbRadius
            val bottom = h - selectionScale * mPressedThumbRadius
            mCircleRadius = (bottom - top) / 2
            val left = w / 2 - mCircleRadius
            val right = w / 2 + mCircleRadius
            RectF(left, top, right, bottom)
        } else {
            val left = selectionScale * mPressedThumbRadius
            val right = w - selectionScale * mPressedThumbRadius
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (progress.toInt() == 99) {
            canvas.drawArc(mOvalRect,
                    if (seekDirection == Direction.COUNTERCLOCKWISE) endAngle else -endAngle,
                    mSweepAngle, false, mStaticPaint)
        }
        canvas.drawArc(mOvalRect,
                if (seekDirection == Direction.CLOCKWISE) startAngle else -startAngle, mSweepAngle,
                false, mStaticPaint)
        canvas.drawArc(mOvalRect,
                if (seekDirection == Direction.CLOCKWISE) startAngle else -startAngle,
                mSecondaryProgressSweepAngle, false, mSecondaryProgressPaint)
        canvas.drawArc(mOvalRect,
                if (seekDirection == Direction.CLOCKWISE) startAngle else -startAngle,
    ...
    override fun onTouch(view: View, motionEvent: MotionEvent): Boolean {/*val x = motionEvent.x - mOvalRect.centerX()
    ...
    private fun obtainStyleAttributes(context: Context, attrs: AttributeSet?) {
        if (attrs != null) {
            val a = context.theme.obtainStyledAttributes(attrs, R.styleable.CircularSeekBar, 0, 0)
            maxProgress = a.getInteger(R.styleable.CircularSeekBar_maxValue, DEFAULT_MAX_PROGRESS)
                    .toLong()

            minProgress = a.getInteger(R.styleable.CircularSeekBar_minValue, DEFAULT_MIN_PROGRESS)
                    .toLong()

            startAngle = a.getFloat(R.styleable.CircularSeekBar_startAngle, DEFAULT_START_ANGLE)

            endAngle = a.getFloat(R.styleable.CircularSeekBar_endAngle, DEFAULT_END_ANGLE)

            mPrimaryColor =
                    a.getColor(R.styleable.CircularSeekBar_primaryColor, DEFAULT_PROGRESS_COLOR)

            mSecondaryColor = a.getColor(R.styleable.CircularSeekBar_secondaryColor,
                    DEFAULT_SECONDARY_PROGRESS_COLOR)

            mStaticColor = a.getColor(R.styleable.CircularSeekBar_staticColor, DEFAULT_STATIC_COLOR)

            mTextColor = a.getColor(R.styleable.CircularSeekBar_textColor, mPrimaryColor)

            format = ProgressFormat.values()[a.getInt(R.styleable.CircularSeekBar_textFormat, 1)]
    ...
    private fun detectTouchAngle(x: Float, y: Float) {
        mProgressSweepAngle = if (seekDirection == Direction.CLOCKWISE) detectClockwise(x, y)
        else detectCounterClockwise(x, y)
        updateThumbPosition()
        calculateProgress()
    }
    ...
    private fun calculateSecondaryProgressAngle() {
        mSecondaryProgressSweepAngle = secondaryProgress * mSweepAngle / (maxProgress - minProgress)
    }
    ...
    private fun updateThumbPosition() {
        ...
        currentThumbX = (mCircleRadius * cos(Math.toRadians(
                (koef * startAngle + mProgressSweepAngle).toDouble())) + deltaX).toFloat()
        currentThumbY = (mCircleRadius * sin(Math.toRadians(
                (koef * startAngle + mProgressSweepAngle).toDouble())) + deltaY).toFloat()
        this.invalidate()
    }

    private fun drawTextProgress(canvas: Canvas) {
        val text = when (format) {
            //ProgressFormat.TIME -> DateUtils.formatElapsedTime(progress / 100)

            ProgressFormat.PLAIN -> "00:$progress"
            else -> progress.toString()
        }
        canvas.drawText(text, mOvalRect.centerX(), mOvalRect.centerY() + mTextPaint.textSize / 2,
                mTextPaint)
    }
...
//    }
    ...
    private fun animateThumbScale(toRadius: Float, isSelected: Boolean) {
        val thumbRadius =
                PropertyValuesHolder.ofFloat(PROPERTY_THUMB_RADIUS, mThumbRadius, toRadius)
        val thumbSelectionRadius = PropertyValuesHolder.ofFloat(PROPERTY_SELECTION_RADIUS,
                //mThumbSelectionRadius,
                if (isSelected) selectionScale * toRadius else 0f)
    ...
    companion object {
        ...
        private const val DEFAULT_STATIC_COLOR = Color.WHITE
        private const val DEFAULT_START_ANGLE = 180f
        private const val DEFAULT_END_ANGLE = 350f
        private const val THUMB_RADIUS = 25f
        private const val SELECTION_SCALE = 1.5f
        private const val PRESSED_THUMB_RADIUS = 25f
